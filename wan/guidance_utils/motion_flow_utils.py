...
def compute_motion_flow(q, k, h=360, w=640, temp=2, nframes=81, argmax=False):
    ...
    A_all = torch.matmul(q[-1, :, :, :], k[-1,:, :, :].transpose(-1, -2)) / torch.sqrt(torch.tensor(q.shape[-1], dtype=q.dtype, device=q.device))
